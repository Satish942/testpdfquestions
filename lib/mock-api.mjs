...
const STORE_DIR = process.env.VERCEL 
  ? path.join(os.tmpdir(), 'mock-stores')
  : path.join(__dirname, '..', 'uploads', 'mock-stores')
...
export async function runFileSearchUpload(ai, input) {
  ...
  // More conservative splitter: only split on Q1), Question 1:, or 1) at the start of a line
  const rawBlocks = rawText.split(/(?:\n|^)\s*((?:Question\s*)?\d+[\.\):]|Q\d+[\.\):]|\d+\s*[\)\:])(?:\s+|$)/gi)
  
  // DIAGNOSTIC (Skipped on Vercel to avoid EROFS)
  if (!process.env.VERCEL) {
     try { fs.writeFileSync(path.join(STORE_DIR, 'latest-raw-parse.txt'), rawText) } catch(e) {}
  }
